...
<head>
    ...
    {!! SEO::generate() !!}
     ...
     if(!empty($data['post']->featured_image)){
         ?>
         <meta property='og:image' content="{{ url('laravel_project/public' . $data['post']->featured_image) }}"/>
         <?php
...
<meta property='og:url' content="https://karimy.ca/blog/{{ $data['post']->slug }}"/>
    ...
    @if($site_global_settings->setting_site_favicon)
        <link rel="icon" type="icon" href="{{ Storage::disk('public')->url('setting/'. $site_global_settings->setting_site_favicon) }}" />
        <!--  sizes="96x96" -->
    @else
...
<script src="{{ asset('frontend/js/subscription_form.js') }}"></script>
